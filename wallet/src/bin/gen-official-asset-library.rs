...
///
/// To generate an official asset library for the Goerli deployment, start the Goerli client (e.g.
...
/// icons. Then run
///
///     gen-official-asset-library -k $SIGNING_KEY_PAIR SYM1 SYM2 ...
///
...
///
/// To generate the library for any other deployment (including possibly a local one) the process is
/// very much the same, only you will change how you launch the wallet GUI according to the
/// deployment, and you may have to use the `--port` argument to `gen-official-asset-library`
/// accordingly.
#[derive(StructOpt)]
struct Options {
