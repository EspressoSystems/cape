...
impl EthPolling {
    ...
    pub async fn check(&mut self) -> Result<u64, async_std::io::Error> {
        ...
        loop {
            ...
            let fetch_max = if fetch_until > self.next_block_to_query + self.max_blocks_to_query {
                self.next_block_to_query + self.max_blocks_to_query
            } else {
                fetch_until
            };
            ...
            for (filter, meta) in new_event {
                ...
                match filter {
                    ...
                    CAPEEvents::FaucetInitializedFilter(filter_data) => {
                        ...
                        let ro_sol: RecordOpeningSol =
                            AbiDecode::decode(filter_data.ro_bytes).unwrap();
                        let ro = ro_sol.into();
                        ...
                        let commit_event = LedgerEvent::Commit {
                            block: cap_rust_sandbox::ledger::CapeBlock::new(vec![
                                transition.clone()
                            ]),
                            block_id: 0,
                            state_comm: 1,
