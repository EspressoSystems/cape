...
contract RootStore {
    ...
    /// @dev constuctor method
    /// @param nRoots maximum number of roots to store
    ...
    /// @param newRoot value of the new root
    function _addRoot(uint256 newRoot) internal {
        _roots[_writeHead] = newRoot;
        _writeHead = (_writeHead + 1) % uint64(_roots.length);
    }

    /// @dev Check if a root value is contained in the store
    /// @param root root value to check against
    /// @return true if the root value is in the store, false otherwise
