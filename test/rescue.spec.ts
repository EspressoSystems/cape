...
describe("Rescue", function () {
  ...
  let contract: RescueNonOptimized;

  beforeEach(async function () {
    // const Contract = await ethers.getContractFactory(contractName);
    const Contract = await ethers.getContractFactory("RescueNonOptimized");
    contract = await Contract.deploy();
    await contract.deployed();
  });

  it("can call .expMod", async function () {
    const tx = await contract.expMod(1, 2, 3);
  ...
  it.only("can call .hash", async function () {
    const tx = await contract.hash(1, 2, 3, { gasLimit: 100_000_000 });
    const receipt = await tx.wait();
    // expect(receipt).to.equal(0);
    // expect().to.equal(
    //   "9084330654817845835997205722524578895477688063555937388415796748573681505264"
